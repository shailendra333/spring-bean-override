...
public class BeanOverrideCheckerViolationsTest {
	...
	private void assertThatContextHasViolations( Class<?> context, BeanOverrideViolation ... violations) {
		assertThat(new BeanOverrideChecker().getBeanOverrideViolations(context),
				Matchers.containsInAnyOrder(violations));
	}
	
	@Test
	public void oneViolationTest() {
		BeanDefinition overridingBeanB = new BeanDefinition("beanB", OneViolationContext.class);
		assertThatContextHasViolations( OneViolationContext.class, 
				new BeanOverrideViolation.NotOverriding(overridingBeanB) );
	}
	...
	public void twoViolationTest() {
		assertThatContextHasViolations( TwoViolationsContext.class, 
				new BeanOverrideViolation.NotOverriding(new BeanDefinition("beanB", TwoViolationsContext.class)),
				new BeanOverrideViolation.ShouldNotOverride( new BeanDefinition("beanA", TwoViolationsContext.class), 
						BaseContext.class ));
	}
